...
```{r modeling w external predictors}
...
# Time series forecasting with external predictors
# Target column index
target_col_index <- which(colnames(combined_train) == "O3")

# Time series auto arima forecasting with external predictor
...
arima_model <- auto.arima(combined_train[, target_col_index], xreg = combined_train[, -target_col_index], 
                          seasonal = TRUE, approximation = FALSE, trace = TRUE, 
                          stationary = FALSE, parallel = TRUE)

# Forecast with the selected ARIMA model
modeled365_combined <- forecast(arima_model, h = 365, xreg = combined_test[, -target_col_index])
```
...
cat("\n\nModeled with External Variables Forecast:\n")
